...
void Graph::BuildFromAdjacencyList(istream& inputStream) {          // Sets edges between nodes by reading adjancency list pairs from inputStream
    ...
    for(int i = 0; i < numberOfEdges; ++i) {
        ...
        --node1; --node2; // deoarece pe infoarena sunt indexate de la 1 nodurile
...
int main() {
    ...
    ifstream inputFile("bellmanford.in"); 
    ofstream outputFile("bellmanford.out");

    int numberOfNodes, numberOfEdges;

    inputFile >> numberOfNodes >> numberOfEdges;

    Graph graph(numberOfNodes, numberOfEdges, true, true);  

    graph.BuildFromAdjacencyList(inputFile);

    vector<int> distances = graph.WeightedDistances(0);
