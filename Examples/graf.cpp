...
int main() {
    ...
    ifstream inputFile("graf.in"); 
    ofstream outputFile("graf.out");

    int numberOfNodes, numberOfEdges, x, y;     // x, y semnificatia din enunt

    inputFile >> numberOfNodes >> numberOfEdges >> x >> y;

    ++numberOfNodes; // Deoarece nodurile sunt indexate de la 1 pe infoarena

    Graph graph(numberOfNodes, numberOfEdges);

    graph.BuildFromAdjacencyList(inputFile);

    vector < int > distanceFromX;
    vector < int > distanceFromY;

    distanceFromX = graph.UnweightedDistances(x);           // Facem doua BFS-uri, unul din x si unul din y
    distanceFromY = graph.UnweightedDistances(y);


    int minim = distanceFromX[y];       // echivalent cu distanceFromY[x];

    int frequency[7503];                // Pentru a gasi nodurile apartenente Tuturor drumurilor optime. 
                                        // In primul rand, un nod se afla pe cel putin un drum minim daca distanceFromX[nod] + distanceFromY[nod] == minim
                                        // contorizam de cate ori intalnim fiecare distanceFromX[nod] , unde nod are proprietatea de mai sus
                                        // Nodurile care sunt la intersectia drumurilor vor avea distanta lor fata de x (sau fata de y) aparuta o singura data 
                                        // (deoarece toate drumurile optime trec prin ele, inseamna ca nu exista un nod alternativ la aceeasi distanta fata de x/y)  
                                        // Deci cautam nodurile de pe drumuri optime pentru care frequency[distanceFromX[nod]] == 1
    for(int i = 0 ; i < numberOfNodes; ++i) {
        frequency[i] = 0;
    }
    ...
    for(int i = 1; i < numberOfNodes; ++i) {
        ...
        if (distanceFromX[i] + distanceFromY[i] == minim) {
            ++frequency[distanceFromX[i]];
        }
    }
    ...
    for(int i = 0; i < numberOfNodes; ++i) {
        if(distanceFromX[i] + distanceFromY[i] == minim && frequency[distanceFromX[i]] == 1) {
            answer.push_back(i);
        }
    }

    outputFile<<answer.size()<<'\n'; 
    for(int node : answer) outputFile<<node<<' ';
    return 0;
}