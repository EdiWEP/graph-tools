...
int main() {
    ...
    Graph graph(numberOfNodes+1, numberOfEdges, false, true);    // +1 deoarece pe infoarena nodurile sunt indexate de la 1 

    graph.BuildFromAdjacencyList(inputFile);

    Graph mst = graph.MinimumSpanningTree(); 

    vector<Graph::Edge> edges = mst.GetAllEdges();

    outputFile<<mst.TotalCost()<<'\n';
    outputFile<<mst.GetNumberOfEdges()<<'\n';
    for(auto edge : edges) {
        outputFile<<edge.source<<' '<<edge.destination<<'\n';
    }
