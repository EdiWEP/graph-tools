...
int main() {
    ...
    ifstream inputFile("disjoint.in"); 
    ofstream outputFile("disjoint.out");

    int numberOfNodes, numberOfOperations;

    inputFile >> numberOfNodes >> numberOfOperations;

    Graph::DisjointSets ds(numberOfNodes + 1);        // +1 deoarece pe infoarena nodurile sunt indexate de la 1

    for(int i = 0; i < numberOfOperations; ++i) {

        int operation, node1, node2;

        inputFile >> operation >> node1 >> node2;

        if(operation == 1) {
            ds.Union(node1, node2);
        }
        else {
            if(ds.Root(node1) == ds.Root(node2)) {
                outputFile<<"DA\n";
            }
            else {
                outputFile<<"NU\n";   
