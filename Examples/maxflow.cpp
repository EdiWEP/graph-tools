...
int Graph::MaximumFlow(unsigned int source, unsigned int sink, FlowHandler& handler) {
    ...
    while(visited[sink]) {
        ...
        for(const auto& edge : adjacencyList[sink]) {  // By checking the paths of all nodes that connect to the sink, we can reduce the number of BFS's that must be performed
