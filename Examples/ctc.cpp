...
void Graph::BuildFromAdjacencyList(istream& inputStream) {           // Sets edges between nodes by reading adjancency list pairs from inputStream
    ...
    for(int i = 0; i < numberOfEdges; ++i) {
        ...
        --node1; --node2;                              // Deoarece nodurile sunt indexate de la 1 pe infoarena/ctc
...
int main() {
    ...
    vector< vector <int> > ssc = graph.StronglyConnectedComponents();

    int sscNumber = ssc.size();
    
    outputFile << sscNumber << '\n';

    for(int i = 0; i < sscNumber; ++i) {
        for(int node : ssc[i]) {
