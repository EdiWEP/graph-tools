...
int main() {
    ...
    ifstream inputFile("royfloyd.in"); 
    ofstream outputFile("royfloyd.out");

    int numberOfNodes, numberOfEdges;

    inputFile >> numberOfNodes;

    Graph graph(numberOfNodes, 0, true, true);

    graph.BuildFromAdjacencyMatrix(inputFile);

    vector<vector<int>> adjMatrix = graph.AllMinimumDistances();

    for(int i = 0; i < numberOfNodes; ++i) {
        for(int j = 0; j < numberOfNodes; ++j) {
            if(adjMatrix[i][j] == Graph::INFINITE) outputFile << 0 << ' ';
            else outputFile << adjMatrix[i][j] << ' ';
        }
        outputFile << '\n';
    }
    return 0;
